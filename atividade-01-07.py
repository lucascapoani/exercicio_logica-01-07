...
while True:
    ...
    if teclado == "1":
        ...
        while True:
            try:
                temperatura = float(input("Temperatura Cº: ").replace(",","."))
                break
            except:
                input("Erro. ")
    ...
    if teclado == "2":
        #ler cidade
        cidade = input("Cidade a ser excluída: ").upper()
    ...
    if teclado == "4":
        ...
        for ind in rangue(len(lista_cidades)):
            print (lista_cidade[ind], "-", lista_temperatura[ind])
